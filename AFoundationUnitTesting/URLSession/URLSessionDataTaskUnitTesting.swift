...
class URLSessionDataTaskUnitTesting: XCTestCase {
    ...
    func testFailureError() {
        ...
        let dataTask = urlSession.urlDataTask(with: request) { (result) in
            switch result {
    ...
    func testSuccessHttpUrlResponseData() {
        ...
        let dataTask = urlSession.urlDataTask(with: request) { (result) in
            ...
            case .success(let urlDataTaskResponse):
                switch urlDataTaskResponse {
                case let .urlResponseWithData(urlResponse, _data):
