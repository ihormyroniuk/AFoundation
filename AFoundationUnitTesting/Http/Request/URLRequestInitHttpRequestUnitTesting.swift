...
class URLRequestInitHttpRequestUnitTesting: XCTestCase {
    ...
    func testInitWithoutheadersAndMessageBody() {
        ...
        XCTAssertTrue(urlRequest.url?.absoluteString == httpRequest.uri, "Unexpected URLRequest object's url \"\(String(describing: urlRequest.url))\" is found while url \"(\(String(describing: httpRequest.uri))\" is expected)")
