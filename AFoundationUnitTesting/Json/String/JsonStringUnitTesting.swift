...
class StringUnitTesting: XCTestCase {
    ...
    func testNotString() {
        ...
        do {
            ...
            XCTFail("Unexpected string \"\(String(describing: string))\" is found while error \(JsonAnyValueIsNotStringError.self) has to be thrown")
        } catch _ as JsonAnyValueIsNotStringError {
